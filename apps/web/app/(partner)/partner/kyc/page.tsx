...
function PhotoUpload({
  label, value, onChange, accept = 'image/*', disabled
...
export default function KYCPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => partnerAPI.kyc().then(r => r.data),
  })
  ...
  const submit = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['partner-kyc'] })
    },
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#1a0533] via-[#2d0a5e] to-[#0f1a4a] border border-violet-500/20 p-5 sm:p-6">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-violet-300/60 text-xs sm:text-sm mt-1">Complete KYC to unlock withdrawals</p>
          </div>
        </div>
      </div>

...
const MENTOR_CATEGORIES = [
  ...
  { value: 'students',   label: 'Student Related' },
...
function TicketCard({ ticket }: { ticket: any }) {
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      setReply('')
      qc.invalidateQueries({ queryKey: ['mentor-tickets'] })
      toast.success('Reply sent')
    } catch { toast.error('Failed to send') } finally { setSending(false) }
  }

  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: '#0f0f1a', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      {open && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }} className="px-5 pb-5">
              ...
              return (
                <div key={i} className={`flex gap-3 ${isAdmin ? '' : 'flex-row-reverse'}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-black ${
                    isAdmin ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/10 text-gray-300'
                  }`}>
                    {isAdmin ? 'A' : 'Me'}
                  </div>
                  <div className={`flex-1 max-w-[80%] ${isAdmin ? '' : 'text-right'}`}>
                    <div className={`inline-block rounded-2xl px-4 py-2.5 text-sm ${
                      isAdmin ? 'bg-indigo-500/10 text-gray-300 rounded-tl-sm' : 'bg-white/8 text-gray-300 rounded-tr-sm'
                    }`}>
                      {m.message}
                    </div>
...
export default function MentorSupportPage() {
  ...
  const [form, setForm] = useState({ subject: '', description: '', category: 'salary', priority: 'medium' })
  const qc = useQueryClient()

  const { data, isLoading } = useQuery({
    queryKey: ['mentor-tickets'],
    queryFn: () => ticketAPI.list().then(r => r.data),
  })

  const createMut = useMutation({
    mutationFn: () => ticketAPI.create({ ...form, userType: 'mentor' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['mentor-tickets'] })
      setShowForm(false)
      setForm({ subject: '', description: '', category: 'salary', priority: 'medium' })
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl p-5 sm:p-6"
        style={{ background: 'linear-gradient(135deg, rgba(79,70,229,0.3) 0%, rgba(139,92,246,0.2) 100%)', border: '1px solid rgba(99,102,241,0.25)' }}>
        <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full blur-3xl pointer-events-none" style={{ background: 'rgba(99,102,241,0.15)' }} />
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <LifeBuoy className="w-5 h-5 text-indigo-300" />
              <span className="text-indigo-300 text-xs font-bold uppercase tracking-wider">Help & Support</span>
            </div>
            <h1 className="text-2xl font-black text-white">Mentor Support</h1>
            <p className="text-indigo-200/50 text-sm mt-0.5">Raise a ticket — our team will respond soon</p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: '#0f0f1a', border: '1px solid rgba(59,130,246,0.2)' }}>
        ...
        <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: '#0f0f1a', border: '1px solid rgba(34,197,94,0.2)' }}>
      ...
      {/* New Ticket Form */}
      {showForm && (
        <div className="rounded-2xl p-5" style={{ background: '#0f0f1a', border: '1px solid rgba(99,102,241,0.3)' }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-white flex items-center gap-2">
              <Plus className="w-4 h-4 text-indigo-400" /> New Support Ticket
            </h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                  ...
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  {MENTOR_CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                </select>
              </div>
              <div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2].map(i => <div key={i} className="h-20 rounded-2xl animate-pulse" style={{ background: '#0f0f1a' }} />)}
        </div>
      ) : tickets.length === 0 ? (
        <div className="text-center py-16 rounded-2xl" style={{ background: '#0f0f1a', border: '1px solid rgba(255,255,255,0.06)' }}>

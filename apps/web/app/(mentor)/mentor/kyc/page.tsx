...
const STATUS_CFG = {
  pending:   { icon: Clock,        label: 'Not Submitted',   color: 'text-gray-400',   bg: 'bg-gray-500/10',  border: 'border-gray-500/20',  desc: 'Submit your KYC documents to enable salary payments' },
  ...
  verified:  { icon: ShieldCheck,  label: 'KYC Verified ✓',  color: 'text-green-400',  bg: 'bg-green-500/10', border: 'border-green-500/25', desc: 'Identity verified — salary will be credited directly to your bank' },
...
function PhotoUpload({ label, value, onChange, accept = 'image/*', disabled }:
  { label: string; value?: string; onChange: (url: string) => void; accept?: string; disabled?: boolean }) {
      ...
      {value ? (
        <div className="relative rounded-xl overflow-hidden border border-dark-600 group">
          ...
          {!disabled && (
            <button onClick={() => inputRef.current?.click()}
              className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 text-white text-xs font-semibold transition-opacity">
        ...
        <button onClick={() => !disabled && inputRef.current?.click()} disabled={disabled || uploading}
          className={`w-full h-28 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-2 transition-all
            ${disabled ? 'border-dark-700 cursor-not-allowed' : 'border-dark-600 hover:border-violet-500/60 hover:bg-violet-500/5 cursor-pointer active:scale-[0.98]'}`}>
          {uploading
            ? <><Loader2 className="w-5 h-5 text-violet-400 animate-spin" /><span className="text-dark-400 text-xs">Uploading...</span></>
      ...
      <div className="relative">
        <div className={`w-20 h-20 rounded-2xl overflow-hidden border-2 border-dark-600 ${uploading ? 'opacity-60' : ''}`}>
          {value
            ...
            : <div className="w-full h-full bg-gradient-to-br from-violet-600 to-purple-700 flex items-center justify-center"><User className="w-8 h-8 text-white/70" /></div>
          }
        </div>
        {!disabled && (
          <button onClick={() => inputRef.current?.click()} disabled={uploading}
            className="absolute -bottom-1.5 -right-1.5 w-7 h-7 rounded-full bg-gradient-to-br from-violet-600 to-purple-700 border-2 border-dark-900 flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-all">
  ...
  return (
    <div>
      ...
      <input value={value}
        ...
        className={`w-full bg-dark-700 border border-dark-600 rounded-xl px-4 py-2.5 text-white placeholder-dark-600 focus:outline-none focus:border-violet-500 text-sm disabled:opacity-50 transition-colors ${upper || mono ? 'tracking-widest font-mono' : ''}`} />
    </div>
  )
}
...
export default function MentorKYCPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['mentor-kyc'],
    queryFn: () => mentorAPI.kyc().then(r => r.data),
  })
  ...
  const submit = useMutation({
    mutationFn: () => mentorAPI.submitKyc(form),
    onSuccess: () => {
      toast.success('KYC submitted successfully!')
      if (form.avatar) updateUser({ avatar: form.avatar } as any)
      qc.invalidateQueries({ queryKey: ['mentor-kyc'] })
    },
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#1a0533] via-[#2d0a5e] to-[#0f1a4a] border border-violet-500/20 p-5 sm:p-6">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute top-4 right-8 w-24 h-24 rounded-full bg-violet-500/15 blur-2xl" />
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-violet-200 text-[11px] font-semibold bg-violet-400/15 border border-violet-400/20 px-2.5 py-1 rounded-full">KYC Verification</span>
            </div>
            <h1 className="text-white text-xl sm:text-2xl font-black">Identity Verification</h1>
            <p className="text-violet-300/60 text-xs sm:text-sm mt-1">Complete KYC to receive salary payments</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={label} className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all
            ${done ? 'bg-green-500/8 border-green-500/20' : 'bg-dark-800 border-dark-700'}`}>
      ...
      {canEdit && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-purple-600 text-white font-black text-base hover:opacity-90 active:scale-[0.98] transition-all shadow-xl shadow-violet-500/25 disabled:opacity-40 disabled:cursor-not-allowed">
          {submit.isPending
            ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</>
            : <><ShieldCheck className="w-5 h-5" /> Submit KYC for Verification</>
          }
        </button>
      )}

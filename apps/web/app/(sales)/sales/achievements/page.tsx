...
import { Trophy, Download, Share2, Lock, Star, X, CheckCircle2, Zap, Coins } from 'lucide-react'
...
async function generateAchievementPoster(opts: {
  ...
  ctx.fillText('TruLearnix Sales Network', W/2, 93)
  ...
  ctx.fillText(`Sales ID: ${opts.salesCode}`, W/2, divY + 50)
  ...
  ctx.fillText('trulearnix.com  ·  Learn. Earn. Grow.', W/2, H-52)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-3 flex gap-2">
          <button onClick={onDownload} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-white text-sm font-bold transition-all hover:opacity-90 active:scale-95" style={{ background: 'linear-gradient(90deg,#0284c7,#4f46e5)', boxShadow: '0 4px 15px rgba(6,182,212,0.3)' }}>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function SalesAchievementsPage() {
  const { user } = useAuthStore()
  const { data, isLoading } = useQuery({
    queryKey: ['sales-achievements'],
    queryFn: () => salesAPI.achievements().then(r => r.data),
  })
  ...
  const handleDownload = useCallback(async (a: any, index: number) => {
    ...
    try {
      const url = await generateAchievementPoster({
        userName: salesUser.name || user?.name || 'Sales Partner',
        avatar: salesUser.avatar || user?.avatar,
        badge: a.badge,
        title: a.title,
        description: a.description,
        earnedAt: a.earnedAt,
        salesCode: salesUser.salesCode || (user as any)?.salesCode || (user as any)?._id?.slice(-6)?.toUpperCase() || '',
        themeIndex: index,
      })
      setPosterUrl(url)
  ...
  const downloadPoster = () => {
    ...
    a.href = posterUrl
    a.download = `trulearnix-sales-${posterAch.title.replace(/\s+/g,'-').toLowerCase()}.png`
    a.click()
  }
  ...
  if (isLoading) return (
    <div className="space-y-5 animate-pulse">
      <div className="h-40 rounded-3xl" style={{ background: 'rgba(6,182,212,0.15)' }} />
      <div className="grid grid-cols-3 gap-3">
        {[...Array(8)].map((_,i) => <div key={i} className="h-36 rounded-2xl bg-dark-800" />)}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      {/* ── Hero ── */}
      <div className="relative overflow-hidden rounded-3xl p-6 sm:p-8" style={{ background: 'linear-gradient(135deg, rgba(6,182,212,0.3) 0%, rgba(37,99,235,0.3) 50%, rgba(79,70,229,0.3) 100%)', border: '1px solid rgba(6,182,212,0.3)' }}>
        <div className="absolute -top-16 -right-16 w-64 h-64 rounded-full blur-3xl pointer-events-none" style={{ background: 'rgba(6,182,212,0.2)' }} />
        <div className="absolute -bottom-12 -left-12 w-48 h-48 rounded-full blur-3xl pointer-events-none" style={{ background: 'rgba(59,130,246,0.15)' }} />

        <div className="relative">
          <div className="flex items-center gap-2 mb-3">
            <div className="px-3 py-1 rounded-full flex items-center gap-1.5" style={{ background: 'rgba(6,182,212,0.2)', border: '1px solid rgba(6,182,212,0.3)' }}>
              <Trophy className="w-3 h-3 text-cyan-300" />
              <span className="text-cyan-300 text-[11px] font-bold uppercase tracking-wider">Sales Achievements</span>
            </div>
          </div>
          <h1 className="text-white text-2xl sm:text-3xl font-black mb-1">Your Achievements</h1>
          <p className="text-sm mb-5" style={{ color: 'rgba(165,243,252,0.6)' }}>Unlock badges, download posters, share your success</p>

          {/* Stats row */}
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[
              { label: 'Unlocked', value: `${earned.length}/${achievements.length}`, icon: Trophy, color: 'text-amber-300' },
              { label: 'Total Earnings', value: `₹${((salesUser.totalEarnings||0)).toLocaleString()}`, icon: Coins, color: 'text-green-300' },
              { label: 'Progress', value: `${pct}%`, icon: Zap, color: 'text-cyan-300' },
          ...
          <div className="rounded-2xl px-4 py-3" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
              <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: 'linear-gradient(90deg,#06b6d4,#3b82f6)' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      {achievements.length === 0 && (
        <div className="text-center py-16 rounded-2xl" style={{ background: 'rgba(15,15,25,0.5)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <p className="text-gray-500 text-sm">Complete sales to unlock achievements</p>
        </div>
      )}
      ...
      {generating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(15,15,30,0.95)', border: '1px solid rgba(6,182,212,0.3)' }}>
            <div className="w-14 h-14 rounded-full border-2 border-cyan-400 border-t-transparent animate-spin mx-auto mb-4" />

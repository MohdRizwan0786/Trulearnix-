...
function EmiStatusInner() {
  ...
  useEffect(() => {
    if (!installmentId || !orderId) { setStatus('failed'); setMessage('Invalid parameters'); return }

    let attempts = 0
    const interval = setInterval(async () => {
      attempts++
      try {
        const { data } = await phonepeAPI.verifyEmiStatus({ installmentId, orderId })
        if (data.success) {
          clearInterval(interval)
          setMessage(data.message || 'EMI payment successful!')
          setStatus('success')
        } else if (attempts >= 6) {
          clearInterval(interval)
          setStatus('failed')
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6"
        ...
        <p className="text-white/60 text-sm">Confirming your EMI payment…</p>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4"
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md">
          ...
          <h2 className="text-3xl font-bold text-white mb-2">EMI Paid!</h2>
    ...
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <div className="text-center max-w-md">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => router.back()} className="px-6 py-3 rounded-xl font-semibold text-white text-sm bg-white/10 border border-white/10">
            Try Again
          </button>
          <Link href="/student/wallet" className="px-6 py-3 rounded-xl font-semibold text-white/60 text-sm">
            My EMI
          </Link>
        </div>
      </div>
...
export default function PhonePeEmiStatusPage() {
  return (
    <Suspense fallback={
